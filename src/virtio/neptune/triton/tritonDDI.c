...
void   APIENTRY tritonCreateGSWithSO_10(D3D10DDI_HDEVICE,
                                        const D3D10DDIARG_CREATEGEOMETRYSHADERWITHSTREAMOUTPUT *, D3D10DDI_HSHADER, D3D10DDI_HRTSHADER, const VOID *);
 ...
 * the slot once per process to produce a DebugView inventory of which
 * entrypoints actually fire. */
...
static SIZE_T APIENTRY tritonStubSize(D3D10DDI_HDEVICE hDevice)
{
    (void)hDevice;
    TR_STUB("Triton<SIZE_T> stub");
    return 0;
}
...
#define TR_STUB_SZ_  ((void *)tritonStubSize)
...
static void tritonFillD3D11_1DeviceFuncs(D3D11_1DDI_DEVICEFUNCS *p)
{
...
static void tritonFillWDDM1_3DeviceFuncs(D3DWDDM1_3DDI_DEVICEFUNCS *p)
{
...
static void tritonFillWDDM2_0DeviceFuncs(D3DWDDM2_0DDI_DEVICEFUNCS *p)
{
...
static void tritonFillWDDM2_1DeviceFuncs(D3DWDDM2_1DDI_DEVICEFUNCS *p)
{
...
static void APIENTRY tritonDestroyDevice(D3D10DDI_HDEVICE hDevice)
{
    ...
    /* Explicit flush before teardown: D3D11 Release implicitly flushes
                                           ...
                                           D3D10DDIARG_CREATEDEVICE *pArgs)
{
     ...
     * same pointer, and the runtime fills the one matching Interface.
     * Every D3D*DDI_CORELAYER_DEVICECALLBACKS version shares pfnSetErrorCb
     * as its first field, and that is the only callback Triton uses (see
     * tritonSetError), so reading it through the 11.0-typed member is
     * correct for all five advertised tiers — not just 11.0/11.1. VBox
     * likewise always stores p11UMCallbacks. */
                                                   ...
                                                   UINT64 *pSupportedDDIInterfaceVersions)
{
    ...
    static const UINT64 kSupportedVersions[] = {
        ...
        /* Top out at the D3D11.1 device DDI, mirroring VBox.  At the WDDM2.x
         * device interface the D3D11 runtime enforces, during device
         * finalization, a depth-stencil-family MSAA consistency rule that
         * this COM-forwarding driver cannot satisfy: it validates the
         * MULTISAMPLE_RENDERTARGET format-support bit on the X-padded depth-
         * read/stencil-read VIEW formats (e.g. R32_FLOAT_X8X24_TYPELESS) by
         * format-support alone, with no CheckMultisampleQualityLevels call --
         * and rejects the device whether we set the bit (a non-target
         * claiming an MSAA target) or clear it (parent/child mismatch with the
         * typeless parent).  The D3D11.1 device DDI instead drives MSAA via
         * CheckMultisampleQualityLevels, which we report consistently, so
         * device creation finalizes.  D3D11.1 still provides the flip-model
         * present DWM needs; advertising WDDM2.x would also require GPU
         * virtual addressing this virtual GPU does not implement. */
    };
                                      ...
                                      const D3D10_2DDIARG_GETCAPS *pArgs)
{
    ...
    case D3D11DDICAPS_3DPIPELINESUPPORT: {
         ...
         * is required to also support 10.1/10.0. Advertising 11.x without
         * the lower levels is an invalid mask, and the runtime rejects the
         * adapter (it falls back to WARP -- which is exactly what DWM did).
         * dxvk-over-Vulkan satisfies 10.x as a subset of its 11.x backend,
         * so report the contiguous range, matching VBox's vboxDXGetCaps. */
        pCaps->Caps =
    ...
    default:
         ...
         * "feature off". This is deliberate and matches VBox's
         * vboxDXGetCaps, which fills only the same cases. The WDDM-tier
         * caps an app may query on the 2.0/2.1 interfaces —
         * D3DWDDM1_3DDICAPS_D3D11_OPTIONS1 (136) / _MARKER (137),
         * D3DWDDM2_0DDICAPS_D3D11_OPTIONS2 (143) / _MEMORY_ARCHITECTURE
         * (145) / _D3D11_OPTIONS3 (152) / _GPUVA_CAPS (153) — all report
         * zero. The dxvk/Vulkan backend's support for these optional
         * features can't be probed at adapter scope (no device exists
         * yet), so zero is the safe, honest answer; reporting them on
         * without backing would be a real bug. */
        break;
    }
    return S_OK;
}
