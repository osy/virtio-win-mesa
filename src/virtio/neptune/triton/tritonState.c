                              ...
                              D3D10DDI_HRTDEPTHSTENCILSTATE hRTState)
{
     ...
     * per-face FrontEnable/BackEnable that can differ. The public D3D11
     * API exposes only StencilEnable, so the runtime keeps Front/Back in
     * lockstep with it (clearing a face only when its ops are already
     * no-ops) — those cases need nothing here. Defensively honor a face
     * being individually disabled by neutralising its ops to the
     * canonical "do nothing" stencil (Func=ALWAYS, all ops=KEEP). VBox
     * drops these enables outright (a documented @todo in its host
     * dxDepthStencilStateCreate). */
 ...
 * ID3D11RasterizerState2 derives from RasterizerState1, so a downcast to
 * the stored type is safe. Falls back to the 11.1 path without Device3
 * (and ConservativeRaster is logged-and-dropped). */
                                    ...
                                    D3D10DDI_HRTRASTERIZERSTATE hRTState)
{
    ...
    /* ID3D11RasterizerState2 derives from RasterizerState1. */
                   ...
                   const D3D10DDI_HRESOURCE *phBuffers, const UINT *pOffsets)
{
     ...
     * NumBuffers. (VBox passes NumBuffers+ClearTargets to its SVGA cmd
     * because that backend does not auto-clear; the end state matches.) */
    (void)ClearTargets;
