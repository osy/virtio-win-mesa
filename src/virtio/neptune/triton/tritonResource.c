                     ...
                     D3D10DDI_HRTRESOURCE hRTResource)
{
    ...
    /* Cross-process sharing is carried by the Neptune shared registry
     * keyed off the KM allocation, not by Win32 shared handles: keep
                   ...
                   D3D10DDI_HRESOURCE hResource, D3D10DDI_HRTRESOURCE hRTResource)
{
    ...
    if (!ax ||
        ...
        ax->Type != VIOGPU_RESOURCE_TYPE_SHARED) {
        TR_LOG("OpenResource: not a shared texture (type=%d)",
               ax ? (int)ax->Type : -1);
        tritonSetError(pD, E_NOTIMPL);
        return;
    }
    const VIOGPU_RESOURCE_SHARED_TEXTURE_OPTIONS *so = &ax->OptionsShared;

    memset(r, 0, sizeof(*r));
    r->hRTResource        = hRTResource;
    r->dim                = D3D10DDIRESOURCE_TEXTURE2D;
    ...
    /* Attach the resource to this process's transport context so the
     * host worker holds its dmabuf before the open names it.  Same-
     * process opens resolve without it, but the rig is harmless. */
...
tritonResourceIsStagingBusy(D3D10DDI_HDEVICE hDevice, D3D10DDI_HRESOURCE hResource)
{
     ...
     * (VBox's vboxDXResourceIsStagingBusy returns TRUE unconditionally —
