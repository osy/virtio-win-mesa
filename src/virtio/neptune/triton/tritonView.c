 ...
 * (GetDesc), so nothing needs to be stored at create time.  Views
 * created through the WDDM 2.0 *View1 paths are recreated as base
 * views: the only extension field (PlaneSlice) is video-format-only. */
HRESULT
                             ...
                             D3D10DDI_HRTRENDERTARGETVIEW hRTRTV)
{
    ...
    /* Pick ARRAY vs non-ARRAY view dimension from the resource: a
     * single-slice view of an array resource (ArraySize=1) still needs
     * an *ARRAY view dimension to expose FirstArraySlice. */
    ...
    case D3D10DDIRESOURCE_TEXTURECUBE:
        /* D3D11 has no RTV cube dimension; use TEXTURE2DARRAY with the
         * cube faces as array slices (each cube contributes 6 faces). */
        d.ViewDimension = D3D11_RTV_DIMENSION_TEXTURE2DARRAY;
                ...
                D3D10DDI_HRTSHADERRESOURCEVIEW hRTSRV)
{
    ...
    D3D11_SHADER_RESOURCE_VIEW_DESC d = {};
    d.Format = pArgs->Format;
    /* Test the resource for arrayness, not the view: a single-slice view
     * of an array resource still needs an *ARRAY view dimension to expose
     * FirstArraySlice. Cube-array iff ArraySize > 6 (cube = 6 faces). */
    const BOOL fIsArray = (r->ArraySize > 1);
                ...
                D3D10DDI_HRTDEPTHSTENCILVIEW hRTDSV)
{
    ...
    case D3D10DDIRESOURCE_TEXTURE2D:
        ...
        break;
    case D3D10DDIRESOURCE_TEXTURECUBE:
        /* D3D11 has no DSV cube dimension; expose as TEXTURE2DARRAY. */
        d.ViewDimension = D3D11_DSV_DIMENSION_TEXTURE2DARRAY;
        d.Texture2DArray.MipSlice        = pArgs->TexCube.MipSlice;
        d.Texture2DArray.FirstArraySlice = pArgs->TexCube.FirstArraySlice;
        d.Texture2DArray.ArraySize       = pArgs->TexCube.ArraySize;
 ...
 *
 * Fallback when Neptune doesn't expose Device3: route through the 11.x
 * create path -- PlaneSlice is silently dropped, harmless for non-planar
 * formats. */
                        ...
                        D3D10DDI_HRTSHADERRESOURCEVIEW hRTSRV)
{
    ...
    /* ID3D11ShaderResourceView1 derives from ID3D11ShaderResourceView. */
