...
/*
 * Stubs: structs.h emits npt_replace_<struct>_handle on both sides;
 * dead code on the guest but still references these.
 */
...
static inline void *
npt_win32_handle_replace(struct npt_dispatch_context *ctx, npt_object_id id)
{
   (void)ctx; (void)id;
   return NULL;
}
